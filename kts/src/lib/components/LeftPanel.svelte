<script>
 
</script>

<aside class="flex flex-col w-[288px] max-h-full h-full ">
<div class="flex flex-col gap-[36px] pl-[8px] pr-[8px] rounded-[20px] pt-[8px] pb-[8px] bg-[#212121]/60">
<div class="flex flex-col gap-[4px] ">
 <div class="flex gap-[12px] items-center pl-[16px] pr-[16px] rounded-[10px] pt-[2px] pb-[2px] bg-[#2F2F2F] w-[272px] h-[71px]">
   <div class="flex flex-col">
      <div class="text-[20px] text-[#F8F8F8] font-medium font-tiktok"><p>сор1ва клан</p></div>
     <div class="flex justify-between items-center gap-[4px]">
       <div class="text-[16px] text-[#6C6C6C] font-medium font-tiktok"><p>группа</p></div>
        <div class="text-[16px] text-[#6C6C6C] font-medium font-tiktok"><p>·</p></div>
         <div class="text-[16px] text-[#6C6C6C] font-medium font-tiktok"><p>4 игрока</p></div>
     </div>
     </div>
   </div>
  </div>
  <div class="flex flex-col gap-[4px]">
     <div class="flex gap-[12px] items-center pl-[16px] pr-[16px] rounded-[10px] pt-[2px] pb-[2px] bg-[#2F2F2F] w-[272px] h-[71px]">
   <div class="flex flex-col">
      <div class="text-[20px] text-[#F8F8F8] font-medium font-tiktok"><p>сор1ва клан</p></div>
     <div class="flex justify-between items-center gap-[4px]">
       <div class="text-[16px] text-[#6C6C6C] font-medium font-tiktok"><p>группа</p></div>
        <div class="text-[16px] text-[#6C6C6C] font-medium font-tiktok"><p>·</p></div>
         <div class="text-[16px] text-[#6C6C6C] font-medium font-tiktok"><p>4 игрока</p></div>
     </div>
     </div>
   </div>
    <div class="flex gap-[12px] items-center pl-[16px] pr-[16px] rounded-[10px] pt-[2px] pb-[2px] bg-[#2F2F2F] w-[272px] h-[71px]">
   <div class="flex flex-col">
      <div class="text-[20px] text-[#F8F8F8] font-medium font-tiktok"><p>Madara0330H</p></div>
     <div class="flex justify-between items-center gap-[4px]">
       <div class="text-[16px] text-[#6C6C6C] font-medium font-tiktok"><p>ваш игровой аккаунт</p></div>
     </div>
     </div>
   </div>
  </div>
  </div>

 
</aside>
